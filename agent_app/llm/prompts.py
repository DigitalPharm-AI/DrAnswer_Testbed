...
def medication_agent_prompt() -> str:
    return (
        ...
        "The AI Server exclusively decides whether to prepare a PRO-CTCAE questionnaire from the authoritative assessment result; "
        "do not independently select or request a questionnaire. "
        ...
        "If the assessment returns suspected=false, explain that the available medication information did not identify the reported symptom as a suspected medication-related effect; "
        "do not claim that a questionnaire is ready and do not request approval to create a side-effect record. "
        ...
        f"Only say that questions are ready when a successful {GET_PRO_CTCAE_QUESTIONNAIRE} result is present in the Tool result history. "
        f"A matched {GET_PRO_CTCAE_QUESTIONNAIRE} result means that standard questions exist for the symptom; it does not establish medication causality. "
