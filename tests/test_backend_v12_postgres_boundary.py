...
TEST_SCHEMA = "dranswer_v12_boundary_test"
TEST_READER_ROLE = "dranswer_v12_boundary_reader"
TEST_READER_PASSWORD = "dranswer-v12-boundary-test-only"
...
def test_postgres_reader_role_can_only_select_versioned_backend_views() -> None:
    ...
    try:
        ...
        assert query_tools.verify_contract() == {
            "ok": True,
        ...
        _assert_statements_denied(
            ...
            (
                ...
                (
                    'UPDATE "ai_v12_chat_messages" '
                    "SET content = content WHERE FALSE"
                ),
                'DELETE FROM "ai_v12_chat_messages" WHERE FALSE',
                'DROP VIEW "ai_v12_chat_messages"',
                'CREATE TABLE "ai_v12_boundary_forbidden" (id INTEGER)',
            ),
        )
        with owner_engine.connect() as connection:
            assert (
                inspect(connection).has_table("ai_v12_boundary_forbidden")
                is False
            )
    finally:
