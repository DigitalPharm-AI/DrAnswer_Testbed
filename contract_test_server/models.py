...
class ChatFeedbackRequest(StrictContractModel):
    ...
    def require_reaction_or_feedback_text(self) -> ChatFeedbackRequest:
        if self.reaction is None and self.feedback_text is None:
            raise ValueError("reaction_or_feedback_text_required")
        return self


class ChatFeedbackAccepted(StrictContractModel):
